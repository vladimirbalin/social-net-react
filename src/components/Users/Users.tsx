...
const Users: React.FC<UsersPropsType> = ({
                                           currentPage, itemsCountPerPage, totalUsersCount,
                                           users, disabledButtons, unFollow, follow, isAuth, portionNumber, setPortionNumber, match
                                         }) =>
  <section className='users'>
    ...
    />
    {users.map(u =>
      <User key={u.id} id={u.id} smallPhoto={u.photos.small} follow={follow} unFollow={unFollow}
            name={u.name} status={u.status} isFollowed={u.followed} disabledButtons={disabledButtons} isAuth={isAuth}/>
    )}
  </section>
