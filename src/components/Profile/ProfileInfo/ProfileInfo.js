...
const ProfileInfo = ({ profile, status, setUserStatusThunk,
                       isOwner, setUserAvatarThunk, isFetchingAvatar, isFetchingProfileInfo, saveProfile, isProfileInfoTransmitted,
                       setProfileInfoTransmitted }) => {
  const [editMode, setEditMode] = useState(false);
  useEffect(() => {
    if(isProfileInfoTransmitted) {setEditMode(false)}; 
  ...
  return (
    <div className='profile__info'>
      ...
      />
      {editMode ? <ProfileInfoListEditMode initialValues={profile}
                                           onSubmit={editModeOff}
                                            profile={profile}/> : <ProfileInfoList profile={profile}
